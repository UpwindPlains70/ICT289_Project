...
point3 rackPos[2];
point3 vecRackToBall;

point3 startingPos[2] = { {77.0, 0.0, -15.0}, {77.0, 0.0, -50.0}  };   // 77.0, 0, -15.0); /// starting location of player1  // (77.0, 0 , -50.0); /// starting location of player2
...
void collisions(int i){
     ...
     if( (vecRackToBall[0] <= 10 && vecRackToBall[0] >= -10) && (vecRackToBall[2] <= 10 && vecRackToBall[2] >= -10) && (vecRackToBall[1] <= 10 && vecRackToBall[1] >= -10) ){
           // printf("COllisions!\n");
            ...
            prevVel[0] = (prevVel[0])* -1;

            prevPos[0] = prevPos[0] - 10;
            prevVel[1] = prevVel[1] + 250.0;
            hasHitBall[i] = TRUE;
