...
/*
Object3D translateObject3D(Object3D *obj, Point3D *CoM)
{
    for(int i = 0; i < obj->nverts; i++)
    {
        obj->vertices[i].x -= CoM.x;
        obj->vertices[i].y -= CoM.y;
        obj->vertices[i].z -= CoM.z;
    }
}
*/
...
/*
Point3D calcCenterOfMass(Object3D obj){
    Point3D CoM = {0,0,0};
    for(int i = 0; i < obj.nverts; ++i)
    {
        CoM.x += obj.vertices[i].x;
        CoM.y += obj.vertices[i].y;
        CoM.z += obj.vertices[i].z;
    }
